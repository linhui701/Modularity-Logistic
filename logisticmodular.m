function [B,stats] = logisticmodular(x, y, Mod_matrix, ...
                                    alpha_set, lambda_list, distr,varargin)
%LOGISTICMODULAR Perform Modularity Logistic Regression Model
%   [B,STATS] = LOGISTICMODULAR(X,Y,...) Performs Modularity Constraint  
%   Logistic regression relating the predictors in X to the responses in Y. 
...
%
%   LOGISTICMODULAR accepts all the command line parameters of the Logistic 
%   function. LOGISTICMODULAR does not calculate standard errors
%   or covariances among the coefficients, as GLMFIT does.
%   LOGISTICMODULAR always calculates an Intercept term. 
%   
...
%     Mod_matrix       Modularity matrix
%     alpha_set        a candidate for hpyer-parameter alpha
%     lambda_list      a list of candidates for hpyer-parameter lambda
%     DISTR            The distributional family for the non-systematic
%                      variation in the responses.  Acceptable values for 
%                      DISTR are 'normal', 'binomial', 'poisson', 'gamma', 
%                      and 'inverse gaussian'.  
%                      By default, the distribution is fit using the 
%                      canonical link ]corresponding to DISTR.  Other link 
%                      functions may be specified using the optional 
%                      parameter 'link'.
%   
...
%                      Dimension 1xL.
%       'Alpha'        The modularity constraint mixing value that was used.
...
[alpha, nLambda, lambdaRatio, lambda, ...
    dfmax, standardize, reltol, cvp, mcreps, maxIter, predictorNames, parallelOptions] ...
     = internal.stats.parseArgs(pnames, dflts, varargin{:});  % maxIter = 10000
...
original_alpha = 1;
[lambdaMax, nullDev, nullIntercept] = computeLambdaMax(X, Y, pwts, original_alpha, standardize, ...
...
penalizedFitPartition = @(x,y,modular_matrix,offset,pwts,n,wlsfit,b,active,mu,eta,sqrtvarFun) ...
    glmIRLSwrapper(x,y,modular_matrix, distr,offset,pwts,dataClass,n, ...
    sqrtvarFun,linkFun,dlinkFun,ilinkFun,devFun,b,active,mu,muLims,wlsfit,nullDev,reltol);

penalizedFit = @(x,y,modular_matrix, wlsfit,b,active,mu,eta) ...
    penalizedFitPartition(x,y,modular_matrix,offset,pwts',nTrials,wlsfit,b,active,mu,eta,sqrtvarFun);

[B,Intercept,lambda,deviance] = ...
    lassoFit(X,Y,Mod_matrix,pwts,lambda,alpha,dfmax,standardize,reltol,maxIter,lambdaMax*totalWeight,ever_active, ...
    penalizedFit,mu,eta,dataClass,userSuppliedLambda,nullDev,nullIntercept);


%----------------------------Linhui-add-----------------------------------%
% remove the returned ALL Zero weights.
idx = find(sum(B)==0);
B(:,idx) = [];
Intercept(idx) = [];
lambda(idx) = [];
...
if ~isequal(cvp,'resubstitution')
    ...
    cvfun = @(Xtrain,Ytrain,Xtest,Ytest) lassoFitAndPredict( ...
        ...
        isCanonical, dlinkFunCanonical,devFun,dataClass,Mod_matrix);
    weights = pwts;
    if isempty(weights)
...
%                      glmIRLS()
% ===================================================
% --------------------Linhui add modular_matrix --------------------------%
function [b,mu,eta,varargout] = glmIRLS(x,y,modular_matrix,distr,offset,pwts,dataClass,N, ...
...
while iter <= iterLim            % iterLim = 100
    iter = iter+1;
    ...
    % --------------------Linhui add modular_matrix ----------------------%
    b_old = b;
    [b,active] = wlsfit(z - offset, x, modular_matrix, sqrtw.^2, b, active);
...
%                      glmIRLSwrapper()
% ===================================================
% --------------------Linhui add Mod_matrix ------------------------------%
function [B,active,varargout] = glmIRLSwrapper(X,Y,Mod_matrix, distr,offset,pwts,dataClass,N, ...
...
% --------------------Linhui add Mod_matrix ------------------------------%
[B,mu,eta,active] = glmIRLS(X,Y,Mod_matrix,distr,offset,pwts,dataClass,N, ...
    sqrtvarFun,linkFun,dlinkFun,ilinkFun,b,active,mu,muLims, ...
    wlsfit,nullDev,devFun,reltol);
...
% --------------------Linhui add modular_matrix --------------------------%
penalizedFit = @(x,y,modular_matrix, wlsfit,b,active,mu,eta) penalizedFitPartition(x,y, modular_matrix, ...
...
% --------------------Linhui add Mod_matrix ------------------------------%
[B,Intercept] = lassoFit(Xtrain,Ytrain, Mod_matrix, ...
...
function [B,Intercept,lambda,varargout] = ...
    lassoFit(X,Y,Mod_matrix,weights,lambda,alpha,dfmax,standardize,reltol,maxIter, ...
...
for i = 1:nLambda
    ...
    wlsfit = @(x,y,modular_matrix,weights,b,active) glmPenalizedWlsWrapper(y,x,modular_matrix,b,active,weights,lam, ...
        alpha,reltol,ever_active,maxIter);
    % --------------------Linhui add Mod_matrix --------------------------%
    [b,active,extras,mu,eta] = penalizedFit(X0,Y0,Mod_matrix,wlsfit,[intercept;b],active,mu,eta);
...
% ===================================================
% --------------------Linhui add Mod_matrix ------------------------------%
function [b_j,active_j,wX2,wX2calculated,shrinkFactor_j] = ...
    cdescentCycleNewCandidates(X0, weights, wX0, wX2, wX2calculated, Y0, ...
    b, active, shrinkFactor_j, threshold, candidates, alpha, Mod_matrix, j)
%-----------------------------Linhui-modified-----------------------------%
lambda = threshold;
n_subj = length(Y0);
r = Y0 - X0*b- (1/X0(:,j))'*lambda*alpha*sum( Mod_matrix(j,:)*b-Mod_matrix(j,j)*b(j) );
bold = b;
...
if margin > 0
    if ~wX2calculated(j)
        ...
        shrinkFactor_j = wX2(j) + shrinkFactor_j;
    end
    b_j = sign(bj) .* margin ./ shrinkFactor_j;
    active_j = active;
    active_j = true;
else
    b_j = b(j);
...
% ===================================================
% ---------------------Linhui add Mod_matrix -----------------------------%
function [b_j,active_j] = ...
    cdescentCycleNoRecalc(X0, wX0, wX2, Y0, b, active, shrinkFactor, threshold, alpha, Mod_matrix, j)
%-----------------------------Linhui-modified-----------------------------%
lambda = threshold;
n_subj = length(Y0);
% vectorize above calculation
r = Y0 - X0*b + (1/X0(:,j))'*1*alpha*sum( Mod_matrix(j,:)*b-Mod_matrix(j,j)*b(j) );
bwX2 = b.*wX2;
bold = b;
...
if active;
    ...
    % Soft thresholding
    if margin > 0
        b_j = sign(bj) .* margin ./ shrinkFactor;
    else
        b_j = 0;
        active_j = false;
    end
...
function [b,varargout] = ...
    penalizedWls(X,Y,Mod_matrix, b,active,weights,lambda,alpha,reltol,maxIter)
    ...
    %----------------------------Linhui-add-------------------------------%
    threshold = lambda;
    shrinkFactor = zeros(1751,1);
    for j = 1:1751
        shrinkFactor(j) = wX2(j) - 1 * alpha * Mod_matrix(j,j);
    end
    %----------------------------Linhui-add-------------------------------%
    ...
    for numIter = 1: maxIter
        ...
        for j = 1:n
            [b_j,active_j] = cdescentCycleNoRecalc(X,wX,wX2,Y, b, active(j),shrinkFactor(j),lambda, alpha, Mod_matrix, j);
            b(j) = b_j;
            active(j) = active_j;
        end
        ...
        if sum(b) ~= 0
            b = b / sqrt(b'*b); % Linhui add, normalized the vector
        end
        %----------------------------Linhui-add---------------------------%
        ...
        if ~any( abs(b(old_active) - bold(old_active)) > reltol * max(1.0,abs(bold(old_active))) )
            ...
            if any(new_candidates)
                ...
                for j = find(new_candidates);
                    [b_j, new_active_j, wX2, wX2calculated, shrinkFactor_j] = ...
                    cdescentCycleNewCandidates(X,weights,wX,wX2,wX2calculated,Y, ...
                    b,active(j),shrinkFactor(j), threshold, new_candidates, alpha, Mod_matrix, j);
                    b(j) = b_j;
                    active(j) = new_active_j;
                    new_active(j) = new_active_j;
                    shrinkFactor(j) = shrinkFactor_j;
                end
                new_active = active;
            else
                new_active = active;
...
% ===================================================
% ---------------------Linhui add Mod_matrix -----------------------------%
function [b,varargout] = glmPenalizedWlsWrapper(X,Y,Mod_matrix,b,active,weights, ...
...
Y = Y - muY;
% ---------------------Linhui add Mod_matrix -----------------------------%
[bPredictors,varargout{1}] = penalizedWls(X0, Y, Mod_matrix, b(2:end), ...
